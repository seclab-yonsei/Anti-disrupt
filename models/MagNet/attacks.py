...
class LinfPGDAttack(object):
    def __init__(self, model=None, device=None, epsilon=0.05, k=10, a=0.01, feat = None, attack_type='gaussian'):
        """
        ...
        # PGD or I-FGSM?
        if attack_type == 'pgd' or attack_type == 'ifgsm':
            self.rand = True
        else:
            self.rand = False
    def perturb(self, X_nat, y):
        ...
        """
        if self.rand:
            X = X_nat.clone() + torch.tensor(np.random.uniform(-self.epsilon, self.epsilon, X_nat.shape).astype('float32')).to(self.device)
            # X = X_nat.clone().detach_() + torch.tensor(np.random.uniform(-self.epsilon, self.epsilon, X_nat.shape).astype('float32')).to(self.device)
        else:
            X = X_nat.clone()
            # X = X_nat.clone().detach_()
            # use the following if FGSM or I-FGSM and random seeds are fixed
            if 'fgsm' in self.attack_type:
                X = X_nat.clone() + torch.tensor(np.random.uniform(-0.001, 0.001, X_nat.shape).astype('float32')).to(self.device)
                # X = X_nat.clone().detach_() + torch.tensor(np.random.uniform(-0.001, 0.001, X_nat.shape).astype('float32')).to(self.device)    
    ...
    def perturb_blur(self, X_nat, y):
        ...
        for i in range(self.k):
            X.requires_grad = True
            output, feats = self.model.forward_blur(X, preproc)
