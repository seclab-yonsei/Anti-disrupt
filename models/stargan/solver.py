...
try:
    ...
    from attacks import add_gausian_noise
...
try:
    ...
    from models.mindiffusion.unet import NaiveUnet
    from models.mindiffusion.ddpm import DDPM
...
class Solver(object):
    ...
    def restore_model(self, resume_iters):
        ...
        if self.defense_model_type != None and 'adv-train' in self.defense_model_type:
            self.model_save_dir = os.path.join(self.model_save_dir, 'adv')
        elif self.defense_model_type != None and 'both' in self.defense_model_type:
            self.model_save_dir = os.path.join(self.model_save_dir, 'adv')
        elif self.defense_model_type != None and 'diffpure' in self.defense_model_type:
            self.model_save_dir = os.path.join(self.model_save_dir, 'adv')
    ...
    def train_adv_both(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            # Black image
            black = np.zeros((x_real.shape[0],3,128,128))
            black = torch.FloatTensor(black).to(self.device)

            # pgd_attack = attacks.LinfPGDAttack(model=self.G, device=self.device, feat=None)
            pgd_attack = attacks.LinfPGDAttack(model=self.G, device=self.device, attack_type=self.attack_type, epsilon=self.test_noise_var)


            # x_real_adv = attacks.perturb_batch(x_real, black, c_trg, self.G, pgd_attack)    # Adversarial training

            x_real_adv, perturb = pgd_attack.perturb(x_real, black, c_trg)
            x_real_adv = x_real + perturb
            # =================================================================================== #
            #                             2. Train the discriminator                              #
            # =================================================================================== #
            ...
            x_fake_adv, perturb = pgd_attack.perturb(x_fake, black, c_org)
            x_fake_adv = x_fake + perturb
    ...
    def test_attack(self, num=None, do_save_images=False, config=None):
        ...
        total_x_fake_list6 = [] # deepfake reform
        ...
        for data_num, (x_real, c_org) in enumerate(data_loader):
            ...
            elif 'ddpm' in self.defense_model_type or 'both' in self.defense_model_type or 'diffpure' in self.defense_model_type:
                with torch.no_grad():
                    defensive_model = DDPM(eps_model=NaiveUnet(3, 3, n_feat=128), betas=(1e-4, 0.02), n_T=1000)

                    defensive_model.load_state_dict(torch.load("./models/mindiffusion/ddpm_celeba.pth",map_location='cuda:0'))
                    defensive_model.to(self.device)

                    # x_ref=defensive_model.sample(x_adv.shape[0], (3, 128, 128), self.device, x_adv, self.ddpm_start_num)
            ...
            if self.test_stable:
                ...
                # x_ref=defensive_model(x_real.shape[0], (3, 128, 128), self.device, x_real, 3)
            x_temp_list = list()
            ...
            for idx, c_trg in enumerate(c_trg_list):
                ...
                if self.attack_type=='gaussian': 
                    ...
                    # gen_noattack = gen_noattack.to(self.device)
                    
                    x_adv = attacks.perturb_batch(x_adv, black, c_org, self.G, fgsm_attack)
                    # x_adv = attacks.perturb_batch(x_adv, gen_noattack, c_org, self.G, fgsm_attack)
                    x_adv = x_adv.to(self.device)
                    ...
                    black = torch.FloatTensor(black).to(self.device)
                    x_adv = x_real.to(self.device)
                    bim_attack = attacks.LinfPGDAttack(model=self.G, device=self.device, attack_type=self.attack_type, epsilon=self.test_noise_var)
                    ...
                    # x_adv = attacks.perturb_batch(x_adv, black, c_org, self.G, bim_attack)
                    # x_adv = attacks.perturb_batch(x_adv, gen_noattack, c_org, self.G, bim_attack)
                    
                    x_adv, perturb = bim_attack.perturb(x_adv, black, c_org)
                    # x_adv, perturb = bim_attack.perturb(x_adv, gen_noattack, c_org)
                    ...
                    # x_adv = attacks.perturb_batch(x_adv, black, c_org, self.G, pgd_attack)
                    # x_adv = attacks.perturb_batch(x_adv, gen_noattack, c_org, self.G, pgd_attack)
                    
                    x_adv, perturb = pgd_attack.perturb(x_adv, black, c_org)
                    # x_adv, perturb = pgd_attack.perturb(x_adv, gen_noattack, c_org)
                    x_adv = x_real + perturb
                ...
                # x_adv, perturb = attacks.perturb(x_real, gen_noattack, c_trg)  
                ...
                elif 'diffpure' in self.defense_model_type:
                    with torch.no_grad():
                ...
                if do_save_images:
                    ...
                    for num in range(len(deepfake_ref)):
                        total_x_fake_list6.append(deepfake_ref[num].cpu())

                    # 공격된 이미지 reform 후 저장
                    for num in range(len(x_real)):
                        if  self.defense_model_type != None and 'diffpure' in self.defense_model_type:
                            with torch.no_grad():
                                x_ori_ref=defensive_model.diffpure(x_real.shape[0], (3, 128, 128), self.device, x_adv, self.ddpm_start_num)
                        elif self.defense_model_type != None and 'ddpm' in self.defense_model_type:
                            with torch.no_grad():
                                x_ori_ref=defensive_model.sample(x_real.shape[0], (3, 128, 128), self.device, x_adv, self.ddpm_start_num)
                        elif self.defense_model_type != None and 'both' in self.defense_model_type:
                            with torch.no_grad():
                                x_ori_ref=defensive_model.sample(x_real.shape[0], (3, 128, 128), self.device, x_adv, self.ddpm_start_num)
                        elif self.defense_model_type != None and 'adv-train' not in self.defense_model_type:
                            with torch.no_grad():
                                x_ori_ref=defensive_model(x_real)
                        else:   # 방어 없으면 노이즈 추가된 이미지 저장
                            x_ori_ref=x_adv
                        try:    
        ...
        if do_save_images:
            utils.createFolder('./dataset/CelebA/pickle/')
            ...
            if self.defense_model_type == None:
                with open('./dataset/CelebA/pickle/disrupt.pkl', 'wb') as f:
                    pickle.dump(total_x_fake_list3, f, protocol=pickle.HIGHEST_PROTOCOL)
            else:
                with open('./dataset/CelebA/pickle/reform.pkl', 'wb') as f:
                    pickle.dump(total_x_fake_list3, f, protocol=pickle.HIGHEST_PROTOCOL)
            ...
            with open('./dataset/CelebA/pickle/deepfake_reform.pkl', 'wb') as f:
                pickle.dump(total_x_fake_list6, f, protocol=pickle.HIGHEST_PROTOCOL)
